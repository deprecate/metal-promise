...
CancellablePromise.resolve = function(opt_value) {
	...
	var promise = CancellablePromise(nullFunction);
...
/**
 * @param {!Array<?(CancellablePromise.<TYPE>|Thenable.<TYPE>|Thenable|*)>} promises
	...
	return new CancellablePromise(function(resolve, reject) {
		...
		for (var i = 0, promise; i < promises.length; i++) {
			...
			CancellablePromise.resolveThen_(
				promise, partial(onFulfill, i), onReject);
		}
	});
};
...
CancellablePromise.allSettled = function(promises) {
	return new CancellablePromise(function(resolve, reject) {
		...
		for (var i = 0, promise; promise = promises[i]; i++) {
			CancellablePromise.resolveThen_(promise,
...
CancellablePromise.firstFulfilled = function(promises) {
	return new CancellablePromise(function(resolve, reject) {
		...
		for (var i = 0, promise; promise = promises[i]; i++) {
			CancellablePromise.resolveThen_(
				promise, onFulfill, partial(onReject, i));
		}
	});
};
...
CancellablePromise.prototype.then = function(opt_onFulfilled, opt_onRejected, opt_context) {
	if (opt_onFulfilled != null && !isFunction(opt_onFulfilled)) {
		throw new Error('opt_onFulfilled should be a function.');
	}

	if (opt_onRejected != null && !isFunction(opt_onRejected)) {
		throw new Error('opt_onRejected should be a function. Did you pass opt_context ' +
...
CancellablePromise.prototype.thenVoid = function(
	...
	this.addCallbackEntry_(CancellablePromise.getCallbackEntry_(
	...
	if (childEntry) {
		if (this.state_ === CancellablePromise.State_.PENDING && childCount === 1) {
			...
			this.executeCallback_(
				childEntry, CancellablePromise.State_.REJECTED, err);
		}
	}
};
...
CancellablePromise.prototype.queueEntry_ = function(entry) {
	if (isFunction(entry.onFulfilled)) {
